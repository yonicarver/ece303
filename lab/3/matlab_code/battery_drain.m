...
i = 1;
tic
while (i && voltage_reading(length(voltage_reading)) > 4.8)
	toc
	pause(i-toc);
	disp(voltage_reading(length(voltage_reading)));
	...
	i = i+1;
			...
			voltage_reading = [voltage_reading ; binblockread(scope,'uint16')];		% appends voltage reading
