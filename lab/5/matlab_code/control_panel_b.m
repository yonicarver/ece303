classdef control_panel_b < matlab.apps.AppBase
...
%             app.gauge_speed.Value = app.light_array(end);
        end
        ...
        function ReadData(app)
                ...
                flushinput(arduino);
                disp('fgetl arduino')
                app.payload_data = fgetl(arduino);
                disp('displaying data')
                disp(app.payload_data);
...
%     end
    ...
    methods (Access = public)
        ...
        function startupFcn(app)
            % STARTUP FUNCTION
            %             estop_status, main_relay, load_cell, optical_counter, temp, coolant, led_normal_op, led_high_temp, led_low_coolant
...
%             time_step = 0.1;
%             app.time = (0 : time_step : 10)';
%             app.RPM_array = strings(length(app.time), 1);
%             
%             app.t = timer;
% %             app.t.TimerFcn = @(~, ~)app.SendData;
%             app.t.TimerFcn = 'flushinput(app.arduino); app.payload_data = fgetl(app.arduino);disp(app.payload_data);';
%             app.t.ExecutionMode = 'fixedRate';
%             app.t.Period = time_step;
%             app.t.BusyMode = 'drop';
...
%             ReadData(app);
%                 app.app.arduino = serial('COM6','Baudrate',9600);
%                 fopen(app.app.arduino);
%                 start(app.t);
%                 payloadd = @(~)app.ReadData;
%                 app.TextArea.Text = string(payloadd);
% 
            if (app.lamp_estop.Color == [1 1 0])    % if estop lamp is red
                app.lamp_estop.Color = 'green';
%                 app.EStopStatusLabel.Text = 'E-Stop Clear';
%                 app.lamp_normal_op.Color = 'white';
...
%             disp('helloworld')
            ...
            time_step = 1;
            app.t = timer;
%             set(app.t, 'TimerFcn',{@ReadData, arduino});
%             app.t.TimerFcn = @(~, ~) app.ReadData;
            app.t.TimerFcn = "flushinput(arduino); payload_data = fgetl(arduino);disp(payload_data);";
%             app.t.TimerFcn = "testing=str2double(fgetl(arduino)); disp(testing)";
%             app.t.TimerFcn = "disp('timer function');";
            app.t.ExecutionMode = 'fixedRate';
            app.t.Period = time_step;
            app.t.BusyMode = 'drop';
...
%             start(app.t);
        ...
        if strcmp(get(app.t,'running'), 'off')
            fopen(arduino);
%             disp('timer is off')
%             disp('starting timer...')
            start(app.t);
            flushinput(arduino);
            payload_data = fgetl(arduino);
            disp(payload_data);

%             fopen(app.arduino);
%             start(app.t);
%             read_data(arduino);
%             app.payload = fgetl(app.arduino);
%             fprintf(app.payload);
        end
        end
        ...
        function button_stopButtonPushed(app, event)
            ...
            app.EStopStatusLabel.Text = 'In E-Stop';
            app.lamp_normal_op.Color = 'black';
            instrreset
            stop(app.t);
            delete(app.t);
            
            if strcmp(get(app.t,'running'), 'on')
                stop(app.t);
                delete(app.t);
                fclose(arduino);
                delete(arduino);
        ...
        function button_resetButtonPushed(app, event)
            ...
            app.EStopStatusLabel.Text = 'Reset';
            app.estop_flag = 0;     % estop is cleared
        end
    ...
    methods (Access = public)
        ...
        function createComponents(app)
            ...
            app.SpeedmphLabel.Position = [147 259 75 22];
            ...
            app.EStopStatusLabel.Position = [377 418 96 22];
